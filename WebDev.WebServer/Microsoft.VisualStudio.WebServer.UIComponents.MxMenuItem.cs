namespace Microsoft.VisualStudio.WebServer.UIComponents
{
    ...
    public class MxMenuItem : MenuItem
    {
        ...
        public MxMenuItem(string text, string helpText, Shortcut shortcut) : base(MenuMerge.Add, 0, shortcut, text, null, null, null, null)
        {
            ...
            {
                this._shortcutText = TypeDescriptor.GetConverter(typeof(Keys)).ConvertToString(null, CultureInfo.CurrentCulture, (Keys) shortcut);
            }
            base.OwnerDraw = true;
        }
        ...
        protected override void OnDrawItem(DrawItemEventArgs e)
        {
            ...
            if (isTopLevel)
            {
                dc.FillRectangle(MxTheme.MenuBarBrush, e.Bounds);
            }
            else
            {
                dc.FillRectangle(SystemBrushes.Menu, e.Bounds);
            }
            if (this.IsSeparator)
            {
                ...
                dc.DrawLine(pen, num, num3, num2, num3);
                pen.Dispose();
            }
            else
            {
                ...
                bool flag4 = (e.State & DrawItemState.HotLight) == DrawItemState.HotLight;
                ...
                if (enabled)
                {
                    if (flag3 || (isTopLevel && flag4))
                    {
                        ...
                        dc.DrawRectangle(SystemPens.Highlight, rect);
                        brush.Dispose();
                        brush2.Dispose();
                    }
                }
                ...
                if (flag2)
                {
                    TextRenderer.DrawText(dc, base.Text, menuFont, bounds, SystemColors.MenuText, TextFormatFlags.RightToLeft);
                }
                else
                {
                    dc.DrawString(base.Text, menuFont, brush3, bounds, format);
                }
                if (!isTopLevel)
                {
                    if (this._shortcutText != null)
                    {
                        ...
                        if (flag2)
                        {
                            TextRenderer.DrawText(dc, this._shortcutText, menuFont, bounds, SystemColors.MenuText, TextFormatFlags.RightToLeft);
                        }
                        else
                        {
                            dc.DrawString(this._shortcutText, menuFont, brush3, bounds, format);
                        }
                    }
                    Rectangle rectangle3 = new Rectangle(e.Bounds.X + 2, e.Bounds.Y + 1, 0x12, 0x12);
                    if (base.Checked && enabled)
                    {
                        this.DrawCheckBackground(dc, rectangle3);
                    }
                    rectangle3.Inflate(-1, -1);
                    this.DrawGlyph(dc, rectangle3, enabled);
                }
                format.Dispose();
                brush3.Dispose();
            }
